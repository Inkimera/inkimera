...
gui_get_children(
  ...
  int num_children = engine_ecs_get_children_of(
    eng,
    node,
    chldrn,
    2
...
/* gui_plugin_deinit */
int
gui_plugin_deinit(gui_plugin_t *plug) {
  free(plug);
  return 0;
}
...
/* gui_plugin_load */
int
gui_plugin_load(engine_t *eng, void *plug) {
  ...
  // Setup
  state_machine_state_t *idle_state = state_machine_add_state(
      &gui_plug->state,
      "idle"
  );
  state_machine_state_t *leader_key_held_state = state_machine_add_state(
      &gui_plug->state,
      "leader_key_held"
  );
  state_machine_state_t *leader_key_released_state = state_machine_add_state(
      &gui_plug->state,
      "leader_key_released"
  );
  state_machine_state_t *command_state = state_machine_add_state(
      &gui_plug->state,
      "command"
  );

  state_machine_event_t *event_leader_held = state_machine_add_event(
    &gui_plug->state,
    (int[2]) { ENGINE_KEY_LEFT_CONTROL, ENGINE_KEY_A },
    (int[2]) { ENGINE_KEY_STATE_DOWN, ENGINE_KEY_STATE_DOWN },
    2
  );
  state_machine_event_t *event_leader_released = state_machine_add_event(
    &gui_plug->state,
    (int[2]) { ENGINE_KEY_LEFT_CONTROL, ENGINE_KEY_A },
    (int[2]) { ENGINE_KEY_STATE_RELEASED | ENGINE_KEY_STATE_UP, ENGINE_KEY_STATE_RELEASED | ENGINE_KEY_STATE_UP },
    2
  );
  state_machine_event_t *event_esc = state_machine_add_event(
    &gui_plug->state,
    (int[1]) { ENGINE_KEY_ESCAPE },
    (int[1]) { ENGINE_KEY_STATE_DOWN | ENGINE_KEY_STATE_PRESSED },
    1
  );
  ...
  state_machine_add_transition(
      ...
      event_leader_held,
      idle_state,
      leader_key_held_state
  );
  state_machine_add_transition(
      &gui_plug->state,
      event_leader_released,
      leader_key_held_state,
      leader_key_released_state
  );
  state_machine_add_transition(
      &gui_plug->state,
      event_leader_held,
...
/* gui_plugin_unload */
int
gui_plugin_unload(engine_t *eng, void *plug) {
...
/* gui_plugin_events */
void
gui_plugin_events(engine_t *eng, void *plug) {
  gui_plugin_t *gui_plug = (gui_plugin_t*)plug;
  ...
  int changed_state = state_machine_process(
    &gui_plug->state,
    (int[3]) { ENGINE_KEY_ESCAPE, ENGINE_KEY_LEFT_CONTROL, ENGINE_KEY_A },
    (int[3]) { escape, ctrl, a },
    3
  );
