...
static void*
_signal_mem_copy(void *dest, const void *src, size_t n) {
  size_t i = 0;
...
static void
_signal_write(void *handle, void *new_data) {
  if (handle && new_data) {
    signal_handle_t *signal_handle = (signal_handle_t*)handle;
    _signal_mem_copy(
      (char*) signal_handle->signal->data,
      (const char*) new_data,
      signal_handle->signal->data_size
    );
...
signal_handle_t*
create_signal_handle(signal_t *signal, void (*callback)(void*)) {
...
void
destroy_signal_handle(signal_handle_t *handle) {
  if (handle) {
    free(handle);
  }
}
